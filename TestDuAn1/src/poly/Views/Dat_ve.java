...
public class Dat_ve extends javax.swing.JFrame {
    ...
    public Dat_ve(NguoiDung nguoiDung) {
        ...
        loadDatVe(trainTicketService.getAll());
        loadVeCuaToi();
    }
...
//        }
//    }
     public void loadDatVe(ArrayList<TrainTicketViewModel> getList) {
        dtm = (DefaultTableModel) tblDatVe.getModel();
        dtm.setRowCount(0);

        for (TrainTicketViewModel n : getList) {
            dtm.addRow(new Object[]{n.getId(),
                // n.getNgaydi(),
                n.getGioKhoiHanh(),n.getGioDen(),
                // n.getDiemdi(), n.getDiemden(),
                n.getTenTau(),
                n.getToa(), n.getViTri(), n.getPrice(), n.getThue()});
        }
    }

    private void loadTimKiem(String diemDi, String diemDen, String ngayDi) {
        ArrayList<TrainTicketViewModel> ve = trainTicketService.timKiem(diemDi, diemDen, ngayDi);
        dtm = (DefaultTableModel) tblDatVe.getModel();
        dtm.setRowCount(0);
        for (TrainTicketViewModel n : ve) {
            dtm.addRow(new Object[]{
                n.getId(),
                // n.getNgaydi(),
                n.getGioKhoiHanh(), n.getGioDen(),
...
//            }
    ...
    private void tbnTimKiemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tbnTimKiemActionPerformed
        ...
        ArrayList<TrainTicketViewModel> listTim = trainTicketService.timKiem(diemDi, diemDen, simpleDateFormat.format(ngayDi));
        if (listTim.size() == 0) {
